     ...
     <div class="row b-article">
      ...
      <div class="col-xs-12 col-md-12 col-lg-12">
       <ul class="row b-metadata">
        ...
        <li class="col-xs-5 col-md-2 col-lg-2"><i class="fa fa-list-alt"></i> <a href="{:'category'.$article['current']['category']['cid']}">{$article['current']['category']['cname']}</a>
         {if condition="!empty($article['current']['tag'])"></if}</li>
        <li class="col-xs-7 col-md-5 col-lg-4 "><i class="fa fa-tags"></i>
         {volist name="article['current']['tag']" id="v"}
          <a class="b-tag-name" href="{:'tag'.$v['tid']}">{$v['tname']}</a>
         {/volist}</li>
       </ul>
      </div>
      <div class="col-xs-12 col-md-12 col-lg-12 b-content-word">
       ...
       <eq name="article['current']['is_original']" value="1">
        <p class="b-h-20"></p>
        <p class="b-copyright"> {$Think.config.COPYRIGHT_WORD}</p>
       </eq>
